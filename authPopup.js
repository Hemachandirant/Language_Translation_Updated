...
async function signIn() {
    ...
    try {
        ...
        console.log("Starting login redirect process...");
...
// Function to check if an interaction is already in progress and clear session if needed
function clearSessionIfNeeded() {
    if (myMSALObj.getAllAccounts().length === 0 && myMSALObj.getActiveAccount()) {
        console.warn("No accounts but an active session exists. Clearing session to avoid conflicts.");
        myMSALObj.logoutRedirect(); // Clear any stale sessions
    }
}

// Run the authentication check when the page is loaded
window.onload = function () {
    clearSessionIfNeeded(); // Ensure no stale interactions
    checkAuthentication();
};
